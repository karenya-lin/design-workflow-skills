...
(() => {
  ...
  const shot = button(bodies[3], '準備截圖', () => {
    if (!selected) return;
    frozen = true;
    status.textContent = '選擇截圖方式。修改單保留選取位置、例外與你輸入的需求；截圖前請檢查可讀性及私密資訊。';
  ...
  function restoreCapture(){screenshotMode=false;captureCard.hidden=true;panel.hidden=false;dock.hidden=true;activateTab(3);shot.focus();draw();}
  function prepareCapture(){
    ...
    screenshotMode=true;hovered=null;pointer=null;panel.hidden=true;dock.hidden=true;
    ...
    captureCard.hidden=false;draw();shield.focus({preventScroll:true});return true;
  }
  ...
  button(screenshotHelp,'Print Screen',()=>nativeCapture('printscreen'));
  button(screenshotHelp,'開啟 Windows 剪取工具',()=>nativeCapture('snipping'));
