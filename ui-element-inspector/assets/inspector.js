...
(() => {
  ...
  style.textContent = `
    ...
    * { box-sizing:border-box; } [hidden] { display:none!important; }
    ...
    .tree-modes { display:flex;gap:2px; }.tree-modes button{font-size:11px;padding:5px 8px;min-height:34px}
    .tree-modes [aria-pressed=true]{background:var(--accent-soft);border-color:var(--accent);color:#115e59;font-weight:700}
    .tree .wrapper-toggle{width:100%;text-align:left;white-space:normal;background:var(--soft);border-style:dashed;font-size:11px;min-height:36px;display:flex;align-items:center;gap:6px}
    .wrapper-toggle .ui-icon{width:20px;height:20px}.wrapper-toggle[aria-expanded=true] .ui-icon{transform:rotate(90deg)}
    .wrapper-layers{padding-left:8px;border-left:2px dashed var(--line)}
    .wrapper-next{padding-left:8px}.wrapper-group{margin:4px 0}
    .structure-details{flex:none;margin:6px 0;max-height:45%;overflow:auto}
    .structure-details p{font-size:11px;color:var(--muted)}.structure-details pre{max-height:100px;overflow:auto}
    .structure-details button{font-size:12px;width:100%}
    ...
    .exception-details > summary { margin-top:8px;padding:12px;border:2px solid #b66b00;border-radius:8px;background:#fff3d5;color:#633900;font-weight:700;cursor:pointer; }
    .exception-details > summary:hover { background:#ffe5a8; }
    .exception-details > summary:focus-visible { outline:3px solid #0e7772;outline-offset:2px; }
  ...
  const selectedBadge=make('span','element-number');outlines[0].append(selectedBadge);
  ...
  const toast=make('div','copy-toast');toast.hidden=true;toast.setAttribute('role','status');toast.setAttribute('aria-live','polite');
  let toastTimer;
  style.textContent+='\n.copy-toast{position:fixed;bottom:24px;right:24px;z-index:2147483647;max-width:calc(100vw - 48px);padding:14px 20px;border-radius:10px;background:#115e59;color:white;box-shadow:0 4px 18px #0003;font:600 14px/1.5 system-ui;pointer-events:none}.element-number{position:absolute;right:0;top:0;transform:translateY(-100%);background:#fbbf24;color:#172438;padding:2px 6px;font:700 14px/1.3 system-ui;border-radius:4px}.match .element-number{background:#7e22ce;color:white}.token-info{white-space:pre-wrap;overflow-wrap:anywhere;max-height:150px;overflow:auto;font:11px/1.6 ui-monospace,monospace}.tokens{flex:none;margin:6px 0}.tokens>summary{cursor:pointer}';
  ...
  const layerHeading=make('div','section-heading');layerHeading.append(make('strong','','頁面圖層'));
  let compactTree=true;
  const treeModes=make('div','tree-modes');treeModes.setAttribute('role','group');treeModes.setAttribute('aria-label','DOM 檢視方式');
  const modeButtons=['精簡','完整 DOM'].map((text,index)=>{
    const control=button(treeModes,text,()=>{
      compactTree=index===0;modeButtons.forEach((b,i)=>b.setAttribute('aria-pressed',String(i===index)));renderTree();
    });control.setAttribute('aria-pressed',String(index===0));return control;
  });layerHeading.append(treeModes);
  ...
  const structureDetails=make('details','structure-details');structureDetails.hidden=true;
  const structureInfo=make('p');structureInfo.setAttribute('data-no-translate','');const structurePaths=make('pre');structurePaths.setAttribute('data-no-translate','');
  const structureLocations=make('details','structure-locations');structureLocations.append(make('summary','','查看容器位置'),structurePaths);
  structureDetails.append(make('summary','','DOM 結構提醒'),structureInfo,
    make('p','','僅依單一子元素的 div 結構提出檢查候選；不能判定容器無用或效能差。'),structureLocations,
    make('p','','精簡只收合工具內的顯示，不改網頁。事件、ref、CSS 與無障礙用途需回原始碼確認。'));
  button(structureDetails,'複製結構檢查需求',()=>selected?.isConnected&&copy(structureRequest()));
  bodies[0].append(structureDetails);
  ...
  const tokenViews=[0,1].map(index=>{
    const details=make('details','tokens');const output=make('pre','token-info');output.setAttribute('data-no-translate','');
    details.append(make('summary','','CSS 變數引用 / CSS variable references'),make('p','muted','僅列可讀且匹配的樣式引用，可能被覆蓋；不是 React 變數或完整 CSS 優先序分析。'),output);
    bodies[index].append(details);return output;
  });
  ...
  shadow.append(shield, ...outlines, hoverOutline, label, panel, dock,captureCard,toast);
  ...
  try {
    ...
    if (mount) {
      ...
      externalShadow.append(externalStyle, panel, dock,captureCard,toast);
  ...
  function cssReferences(node){
    const found=new Set();let incomplete=false,scanned=0;
    function declaration(style){
      // CSSOM expands shorthand names, but var() shorthands can leave longhand values empty.
      const properties=new Set([...style,...Array.from(style.cssText.matchAll(/(?:^|;)\s*([\w-]+)\s*:/g),match=>match[1])]);
      for(const property of properties){
        // Do not enumerate arbitrary custom-property values or copy declaration values.
        if(property.startsWith('--'))continue;
        for(const match of style.getPropertyValue(property).matchAll(/var\(\s*(--[\w-]+)/g)){
          if(found.size>=40){incomplete=true;return;}
          found.add(property+' ← '+match[1]);
        }
      }
    }
    function rules(list){
      for(const rule of list){
        if(++scanned>2000){incomplete=true;return;}
        if(rule.type===CSSRule.STYLE_RULE){
          try{if(node.matches(rule.selectorText))declaration(rule.style);}catch{incomplete=true;}
          if(rule.cssRules?.length)incomplete=true; // Nested selectors require cascade-aware resolution.
        }else if(rule.type===CSSRule.MEDIA_RULE){if(matchMedia(rule.conditionText).matches)rules(rule.cssRules);}
        else if(rule.type===CSSRule.SUPPORTS_RULE){if(CSS.supports(rule.conditionText))rules(rule.cssRules);}
        else if(rule.type===CSSRule.IMPORT_RULE||rule.cssRules?.length)incomplete=true;
      }
    }
    declaration(node.style);
    for(const sheet of [...document.styleSheets,...(document.adoptedStyleSheets||[])]){
      if(sheet.disabled||sheet.media?.mediaText&&!matchMedia(sheet.media.mediaText).matches)continue;
      try{rules(sheet.cssRules);}catch{incomplete=true;} // Never fetch around CSSOM access controls.
      if(scanned>2000)break;
    }
    return {items:[...found],incomplete};
  }
  function draw() {
    ...
    const number=matches.indexOf(selected);
    selectedBadge.textContent=number>=0?'#'+(number+1)+' ✓':'✓';
    selectedBadge.title=locale.translate('已選取 / Selected');
    selectedBadge.setAttribute('aria-label',locale.translate('已選取 / Selected')+(number>=0?' #'+(number+1):''));
  ...
  function describe() {
    ...
    const tokens=cssReferences(selected);
    const tokenText=(tokens.items.length?tokens.items.join('\n'):locale.translate('未找到直接引用；不代表沒有使用變數。'))+'\n'+locale.translate('可能不完整：繼承、覆蓋、巢狀或無法讀取的樣式需另查 source。');
    tokenViews.forEach(view=>{view.textContent=tokenText;view.dataset.incomplete=String(tokens.incomplete);});
    ...
    report.value = locale.translate('請修改此元素 / Please update this element:')+`\nSelector: ${path}\n`
      ...
      + 'CSS variable references (candidates, not cascade proof):\n'+tokenText+'\n'
  ...
  function clearMatches() {
    ...
    externalHost?.dispatchEvent(new CustomEvent('inspector:matches',{bubbles:true,composed:true,detail:{items:[],current:-1}}));
    draw();
  }
  function matchSummary() {
    ...
    externalHost?.dispatchEvent(new CustomEvent('inspector:matches',{bubbles:true,composed:true,detail:{current:matchIndex,items:matches.map((node,index)=>({index,name:identity(node),excluded:exceptions.has(node),available:node.isConnected&&node.getClientRects().length>0}))}}));
  }
  function visitNumber(index){
    if(!Number.isInteger(index)||!matches[index]?.isConnected)return;
    matchIndex=index;matches[index].scrollIntoView({block:'center',behavior:'instant'});
    pick(matches[index],true);matchSummary();
  }
  if(externalHost)on(externalHost,'inspector:visit',event=>visitNumber(event.detail?.index));
  ...
  function highlight(query, method) {
    ...
    matchBoxes = matches.map((node,index) => {
      const box = make('div', 'outline match');
      box.append(make('span','element-number',String(index+1)));
  ...
  function isWrapper(node){
    // This is a display heuristic, never a proof that a wrapper can be removed.
    return !!node&&tag(node)==='div'&&!node.shadowRoot&&node.children.length===1
      &&[...node.attributes].every(attr=>['class','style'].includes(attr.name))
      &&![...node.childNodes].some(child=>child.nodeType===3&&child.textContent.trim());
  }
  function wrapperChain(start){
    const nodes=[];let child=start;
    while(isWrapper(child)&&nodes.length<40){nodes.push(child);child=child.firstElementChild;}
    return {nodes,child};
  }
  function structureContext(){
    let start=selected,depth=0,parent=selected?.parentElement,walks=0;
    while(parent&&depth<200){depth++;parent=parent.parentElement;}
    while(isWrapper(start?.parentElement)&&walks++<40)start=start.parentElement;
    const wrappers=wrapperChain(start).nodes;
    const paths=wrappers.length>=2?wrappers.map(node=>selector(node)):[];
    const t=text=>locale.translate(text);
    return {paths,summary:t('距離 html 的祖先層數：{0}').replace('{0}',String(depth)+(parent?'+':''))+' · '
      +(paths.length?t('連續包裝 div：{0} 層（本次最多 40）').replace('{0}',String(paths.length)):t('此處未找到符合收合規則的連續包裝層，不代表無法優化。'))};
  }
  function structureRequest(){
    const context=structureContext();
    return locale.translate('請檢查此處 DOM 是否可精簡，先讀原始碼，不要直接刪除容器。')+'\nSelector: '+selector(selected)+'\n'
      +context.summary+'\n'+context.paths.join('\n')+'\n'
      +locale.translate('確認各層的排版、RWD、事件、ref、CSS selector、焦點與無障礙用途。只有純分組才考慮 Fragment；有必要用途就保留。修改後驗證外觀及互動，不以層數判定效能。')
      +(request.value.trim()?'\n'+locale.translate('需求 / Requested change: \n')+request.value.trim():'');
  }
  function describeStructure(){
    if(!selected?.isConnected){structureDetails.hidden=true;return;}
    const structure=structureContext();structureDetails.hidden=false;structureInfo.textContent=structure.summary;
    structurePaths.textContent=structure.paths.join('\n');structureLocations.hidden=!structure.paths.length;
  }
  on(window,'workflow:language',describeStructure);
  on(structureDetails,'toggle',()=>requestAnimationFrame(()=>{if(!dead)revealTreeSelection();}));
  function revealTreeSelection(){
    const row=treeRows.get(selected)?.row;if(!row)return;
    const rect=row.getBoundingClientRect(),bounds=tree.getBoundingClientRect();if(!rect.height||!bounds.height)return;
    if(rect.bottom>bounds.bottom)tree.scrollTop+=rect.bottom-bounds.bottom;
    else if(rect.top<bounds.top)tree.scrollTop+=rect.top-bounds.top;
  }
  function syncTree(reveal=false) {
    if(reveal && selected){
      const path=parents(selected,100).reverse();
      path.forEach(node=>{const entry=treeRows.get(node);if(entry?.reveal){if(node===selected)entry.reveal();}else if(entry&&node!==selected)entry.expand(true);});
    }
  ...
  function renderTree() {
    ...
    function branch(node,container,parentEntry=null,wrapperOnly=false,skipGrouping=false) {
      const group=compactTree&&!wrapperOnly&&!skipGrouping?wrapperChain(node):{nodes:[]};
      if(group.nodes.length>=2){
        const item=make('div','wrapper-group'),layers=make('div','wrapper-layers'),next=make('div','wrapper-next');layers.hidden=true;
        const toggle=button(item,'',()=>entry.expand(layers.hidden));
        toggle.append(icon('m9 5 7 7-7 7'),make('span','','⋯ '+group.nodes.length+' 層包裝容器'));
        toggle.className='wrapper-toggle';toggle.setAttribute('aria-expanded','false');
        toggle.title='展開查看各層；不是刪除建議。';
        const entry={name:toggle,parentEntry,expand(open){layers.hidden=!open;toggle.setAttribute('aria-expanded',String(open));}};
        group.nodes.forEach(wrapper=>{const row=branch(wrapper,layers,entry,true);row.reveal=()=>entry.expand(true);});
        // Long chains continue through a normal lazy branch instead of eagerly traversing the page.
        item.append(layers,next);container.append(item);branch(group.child,next,entry,false,group.nodes.length===40);
        toggle.addEventListener('keydown',event=>{
          if(event.key==='ArrowRight'){event.preventDefault();entry.expand(true);layers.querySelector('.tree-name')?.focus();}
          if(event.key==='ArrowLeft'){event.preventDefault();entry.expand(false);}
          if(['ArrowUp','ArrowDown'].includes(event.key)){
            event.preventDefault();const visible=[...tree.querySelectorAll('.tree-name,.wrapper-toggle')].filter(n=>n.getClientRects().length);
            const index=visible.indexOf(toggle);visible[Math.max(0,Math.min(visible.length-1,index+(event.key==='ArrowDown'?1:-1)))]?.focus();
          }
        });
        return entry;
      }
      const item=make('div');const row=make('div','tree-row');
      const nodes=wrapperOnly?[]:[...node.children].filter(n=>n!==host&&!['script','style','link','meta'].includes(tag(n)));
      ...
      row.addEventListener('keydown',event=>{
        ...
        const visible=[...tree.querySelectorAll('.tree-name,.wrapper-toggle')].filter(n=>n.getClientRects().length);
    ...
    syncTree(true);
    // An explicit refresh/view switch should keep its selected row in the tree viewport.
    revealTreeSelection();
  }
  ...
  async function copy(text) {
    ...
    try {
      ...
      if (!dead) {
        status.textContent = '已複製。下一步：切到 AI 對話，貼上並送出。 / Copied: paste into your AI chat. 分享前確認無私密資訊。';
        toast.textContent=locale.translate('已複製！可以貼給 AI。');toast.hidden=false;
  ...
  function prepareCapture(){
    ...
    const scope=make('pre','capture-scope',matchQuery+'\n'+excluded.map(node=>'#'+(matches.indexOf(node)+1)+' '+selector(node)).join('\n'));
  ...
  on(window, 'workflow:language', () => {
    schedule();
    if(!selected?.isConnected)return;
    const tokens=cssReferences(selected);
    const text=(tokens.items.length?tokens.items.join('\n'):locale.translate('未找到直接引用；不代表沒有使用變數。'))+'\n'+locale.translate('可能不完整：繼承、覆蓋、巢狀或無法讀取的樣式需另查 source。');
    tokenViews.forEach(view=>{view.textContent=text;});
    // Preserve the editable report and request when changing language.
  });
