...
**DOM ラッパー：** ① 選択 → 簡略／完全な DOM。グループを展開すると各層を確認できます。DOM 構造の注意点 → 構造の確認依頼をコピー → 内容を確認して AI に貼り付けてください。[英語・繁体字の画像と制限](../ui-element-inspector/references/dom-structure.md)。

ページまたは DOM ツリーで要素を選ぶと、RWD 設定が自動的に折りたたまれます。ホバーだけでは折りたたまれません。サイズを変えるときは RWD 行を再度開いてください。選択と入力した変更内容は保持されます。

**アイコンの使い方：** ↻ はレイヤーを更新（ページは再読み込みしません）、− はパネルを折りたたむ、× は閉じる、⇄ は幅と高さを入れ替えます。RWD 行の右矢印で設定を開閉できます。ホバーで名前を表示、Tab と Enter／スペースでも操作できます。ツールバーボタンは 44px、アイコンは 22px。手順、言語、除外、変更内容のコピーは文字を残しています。[新しい画像](VISUAL-GUIDE.md#larger-icon-controls)。

**現在の操作：** 既定の言語項目は **言語** と表示され、自動判定を使います。デモの開始ボタンで外側パネルの作業画面が開きます。「範囲」では番号付き目盛りで要素に移動できます。「コピー対象」は含める要素だけを表示します。琥珀色の除外ボタンで変更しない要素を選べます。[検証状況](RELEASE-STATUS.md)。
...
撮影は、変更内容を入力 → スクリーンショットを選ぶと、変更メモと枠線がすぐ表示されます。対象と読める変更メモを一緒に撮影。長文は複数枚か全文コピーを添えて、共有前に個人情報を確認してください。
...
ポートは固定ではありません。自分で指定するか、AI に空きポートの選択と実際の URL の報告を依頼できます。`--port` にはその番号を明記。既存の承認済みプレビューを再利用し、他人のプロセスを終了しないでください。


許可された同一オリジンの作業画面では、プレビューにカーソルを合わせるとインスペクターと DOM ツリーが自動で開きます。開始・再開ボタンは不要です。他の要素へのホバーは選択と変更内容を保持し、クリックで選択を変更します。狭い画面では RWD を展開すると設定できます。
