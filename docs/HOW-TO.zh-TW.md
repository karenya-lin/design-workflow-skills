...
**DOM 包裝層：** ① 選元素 → 精簡／完整 DOM；展開包裝列可逐層指認。DOM 結構提醒 → 複製結構檢查需求 → 審閱後貼給 AI。[逐步截圖與限制](../ui-element-inspector/references/dom-structure.md)。

點畫面或 DOM 選取元素後，RWD 設定會自動收合；單純 Hover 不會。需要改尺寸時再點 RWD 列展開，選取與手寫需求都保留。

**圖示怎麼用：** ↻ 更新圖層樹（不重整網頁）、− 收合面板、× 關閉、⇄ 交換寬高。RWD 整列可點，右側箭頭表示展開／收合。滑鼠停在圖示上會顯示名稱，也能用 Tab 移到按鈕，再按 Enter／空白鍵。工具列按鈕有 44px 點擊範圍、22px 圖示；步驟、語言、例外與「複製修改需求」保留文字。[新版圖解](VISUAL-GUIDE.md#larger-icon-controls)。

**新版控制：** 預設語言選項顯示「語言」（自動判定）。示範頁啟動會進外側面板工作區。在「確認範圍」點編號刻度找同類，切「本次」看這次包含的項目，琥珀色「例外」可勾選不要修改的項目。[已驗與待驗狀態](RELEASE-STATUS.md)。
