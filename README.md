...
This repository ships eighteen sibling skill directories, but **you do not need to install all eighteen**. Keep the checkout intact for documentation and tests, then copy only the skill directories required by your task, preserving each selected skill's own `references/`, `scripts/` and `assets/`.
